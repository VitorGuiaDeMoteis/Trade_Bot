...
def upgrade() -> None:
    ...
    op.create_table('broker_orders',
        ...
        sa.Column('filled_quantity', sa.Numeric(precision=28, scale=10), nullable=False, server_default='0'),
        sa.Column('last_reconciled_at', sa.DateTime(timezone=True), nullable=False),
        sa.ForeignKeyConstraint(['order_id'], ['paper_orders.order_id'], name='fk_broker_orders_order_id'),
    ...
    op.create_table('broker_fills',
        ...
        sa.ForeignKeyConstraint(['order_id'], ['broker_orders.order_id'], name='fk_broker_fills_order_id'),
...
def downgrade() -> None:
    op.add_column('paper_fills', sa.Column('broker_fill_id', sa.VARCHAR(length=64), autoincrement=False, nullable=True))
    ...
    op.add_column('paper_orders', sa.Column('last_reconciled_at', sa.DateTime(timezone=True), autoincrement=False, nullable=True))
    op.add_column('paper_orders', sa.Column('broker_status', sa.VARCHAR(length=32), autoincrement=False, nullable=True))
    op.add_column('paper_orders', sa.Column('client_order_id', sa.VARCHAR(length=64), autoincrement=False, nullable=True))
    op.add_column('paper_orders', sa.Column('broker_order_id', sa.VARCHAR(length=64), autoincrement=False, nullable=True))
