...
from research.momentum_v1.data import get_file_hash, validate_artifacts
...
class ResearchPaths:
    ...
    def default(cls) -> ResearchPaths:
        ...
        return cls(root=root, artifacts=root / ".artifacts" / "research" / "momentum_v1")

    @property
    def config(self) -> Path:
        return self.root / "research" / "momentum_v1" / "config.json"
...
def create_seal(paths: ResearchPaths) -> dict[str, object]:
    ...
    seal_data: dict[str, object] = {"protocol_seal_hash": hash_object(manifest), "manifest": manifest}
...
def verify_seal(paths: ResearchPaths) -> dict[str, object]:
    ...
    if canonical_json(current) != canonical_json(sealed_state):
        raise ValueError("HOLDOUT_INVALIDATED: sealed source, config, data, or dependencies changed")
    return seal_data
