...
async def get_broker_portfolio(request: Request, response: Response) -> PaperPortfolio:
    ...
    with engine.begin() as conn:
        ...
        snapshot = conn.execute(
            select(broker_portfolio_snapshots).where(broker_portfolio_snapshots.c.provider == "alpaca")
        ).mappings().first()
        ...
        b_orders = conn.execute(
            select(broker_orders, paper_orders.c.symbol, paper_orders.c.side, paper_orders.c.run_id, paper_orders.c.signal_id, paper_orders.c.risk_decision_id)
            .select_from(j)
            .order_by(broker_orders.c.last_reconciled_at.desc()).limit(100)
        ).mappings().all()
        ...
        b_fills = conn.execute(
            select(broker_fills).order_by(broker_fills.c.filled_at.desc()).limit(100)
        ).mappings().all()
        fills = []
        for f in b_fills:
