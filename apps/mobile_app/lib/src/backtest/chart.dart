...
class _EquityCurveChartState extends State<EquityCurveChart> {
  ...
  int get selected => widget.selectedIndex ?? localSelected ?? (widget.curve.length - 1);
  ...
  Widget build(BuildContext context) {
    ...
    final minimum = widget.curve.map((c) => double.parse(c.equity)).reduce(math.min);
    final maximum = widget.curve.map((c) => double.parse(c.equity)).reduce(math.max);
          ...
          children: [
            ...
            LayoutBuilder(
              builder: (context, constraints) {
                return GestureDetector(
                  onTapDown: (details) {
                    final fraction = ((details.localPosition.dx - 4) / (constraints.maxWidth - 8)).clamp(0.0, 0.999999);
            ...
            Wrap(
              ...
              children: [
                ...
                _Value('Drawdown', frame.drawdown, color: double.parse(frame.drawdown) > 0 ? Colors.red : null),
              ],
            ),
          ],
        ),
...
class _EquityPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final maximum = curve.map((c) => double.parse(c.equity)).reduce(math.max);
    ...
    final path = Path();
    double peak = double.parse(curve.first.equity);
    ...
    if (selected >= 0 && selected < curve.length) {
      ...
      canvas.drawLine(
        ...
        Paint()..color = cursor.withValues(alpha: 0.45)..strokeWidth = 1,
      );
      canvas.drawCircle(Offset(x, y(double.parse(curve[selected].equity))), 4, Paint()..color = line);
    }
  }
