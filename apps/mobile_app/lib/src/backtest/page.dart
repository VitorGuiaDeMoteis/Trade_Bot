...
class _BacktestPageState extends State<BacktestPage> {
  ...
  void initState() {
    super.initState();
    controller = widget.controller ?? BacktestController(api: HttpBacktestApi(const String.fromEnvironment('API_BASE_URL')));
    if (widget.controller == null || controller.listState == BacktestState.loading && controller.summaries.isEmpty) {
  ...
  Widget _buildList() {
    ...
    if (controller.listState == BacktestState.error) {
      return Center(child: Text(controller.errorMessage ?? 'Erro desconhecido.'));
    }
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            subtitle: Text('Hash: ${summary.resultHash.length > 8 ? summary.resultHash.substring(0, 8) : summary.resultHash}...'),
            trailing: Icon(
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              ...
              onPressed: () async {
                final url = Uri.parse(controller.api.getExportUrl(report.resultHash));
                await launchUrl(url);
              },
            ),
          ],
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        Text('Métricas Financeiras', style: Theme.of(context).textTheme.titleLarge),
        const SizedBox(height: 8),
        Wrap(
          spacing: 24,
          runSpacing: 16,
          children: [
            _MetricValue('Retorno', '${report.metrics.returnPct}%', isProfit ? Colors.green : Colors.red),
            _MetricValue('Patrimônio final', report.metrics.finalEquity),
            _MetricValue('P&L líquido', report.metrics.totalPnlNet, isProfit ? Colors.green : Colors.red),
            _MetricValue('Drawdown máximo', report.metrics.maxDrawdown, Colors.red),
            _MetricValue('Operações encerradas', '${report.metrics.closedTrades}'),
            _MetricValue('Win rate', report.metrics.winRatePct != null ? '${report.metrics.winRatePct}%' : 'N/A'),
            _MetricValue('Profit factor', report.metrics.profitFactor ?? 'N/A'),
            _MetricValue('Lucro médio', report.metrics.averageProfit ?? 'N/A', Colors.green),
            _MetricValue('Perda média', report.metrics.averageLoss ?? 'N/A', Colors.red),
            _MetricValue('Fees', report.metrics.fees),
            _MetricValue('Slippage', report.metrics.slippage),
            _MetricValue('Posições abertas ao fim', '${report.metrics.openPositions}'),
          ],
        ),
        ...
        Wrap(
          ...
          children: [
            ...
            _MetricValue('Quantidade de frames', '${report.equityCurve.length}'),
        ...
        child: Column(
          children: [
            Container(
              ...
              child: const Text('MODO BACKTEST - HISTÓRICO', style: TextStyle(fontWeight: FontWeight.bold)),
            ),
            const SizedBox(height: 16),
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final color = pnl > 0 ? Colors.green : (pnl < 0 ? Colors.red : Colors.grey);
            ...
            child: Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    Flexible(child: Text('${trade.symbol} - $result', style: TextStyle(color: color, fontWeight: FontWeight.bold, fontSize: 16))),
                    Flexible(child: Text('P&L Líquido: ${trade.netPnl}', style: TextStyle(color: color, fontWeight: FontWeight.bold))),
                  ],
                ),
                const Divider(),
                Row(
                  ...
                  children: [
                    Flexible(
                      child: Column(
                        ...
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('BUY', style: TextStyle(color: Colors.blue[300])),
                    ...
                    Flexible(
                      child: Column(
                        ...
                        children: [
                          Text('SELL', style: TextStyle(color: Colors.purple[300])),
  ...
  void _togglePlay() {
    if (_isPlaying) {
      ...
      _timer = Timer.periodic(const Duration(milliseconds: 500), (timer) {
        if (widget.controller.replayIndex < widget.controller.currentReport!.equityCurve.length - 1) {
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (context, _) {
        ...
        final outcomes = report.outcomes.where((o) => o.executedAt.isAtSameMomentAs(frame.timestamp)).toList();
        ...
        return Column(
          children: [
            Container(
              ...
              color: Colors.blueGrey[800],
              width: double.infinity,
              child: const Text('REPLAY VISUAL - BACKTEST', style: TextStyle(fontWeight: FontWeight.bold)),
            ),
            Expanded(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Wrap(
                              ...
                              runSpacing: 12,
                              children: [
                                _MetricValue('Timestamp', utcLabel(frame.timestamp)),
                      ...
                      Card(
                        child: Padding(
                          ...
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text('Eventos neste candle', style: Theme.of(context).textTheme.titleMedium),
                              const Divider(),
                              ...outcomes.map((o) => ListTile(
                                title: Text('${o.symbol} - ${o.status}'),
                                subtitle: Text('Reason: ${o.reason} | Price: ${o.referencePrice} | Qty: ${o.quantity}'),
                              )),
                            ],
                          ),
                        ),
              ...
              child: Row(
                ...
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: widget.controller.replayIndex < report.equityCurve.length - 1 ? () {
                      _timer?.cancel();
                      setState(() => _isPlaying = false);
                      widget.controller.advanceReplay();
                    } : null,
                  ...
                  IconButton(
                    ...
                    onPressed: widget.controller.replayIndex < report.equityCurve.length - 1 ? () {
                      _timer?.cancel();
                      setState(() => _isPlaying = false);
                      widget.controller.setReplayIndex(report.equityCurve.length - 1);
                    } : null,
                  ),
                ],
              ),
