...
def provider(handler=None, **kwargs):
    def default(request):
...
def test_history_decimal_utc_closed_identity_and_endpoint():
    requests = []
    ...
    bar = asyncio.run(provider(handler).get_historical_candles("SPY", "1h"))[0]
...
def test_history_respects_single_requested_symbol(symbol):
    seen = []
    ...
    bars = asyncio.run(provider(handler).get_historical_candles(symbol, "1h"))
    assert seen == [symbol] and {bar.symbol for bar in bars} == {symbol}


@pytest.mark.parametrize("timeframe", ["1m", "5m", "15m", "1d", "1Hour", "bad"])
def test_unsupported_timeframe_is_explicit(timeframe):
...
def test_partial_or_grace_period_bars_are_not_emitted(opened):
    def handler(request):
        return httpx.Response(200, json={"bars": {"SPY": [{**RAW, "t": opened}]}})

    assert asyncio.run(provider(handler).get_historical_candles("SPY", "1h")) == []
...
def test_rest_errors_are_safe_and_classified(status, code, retryable, caplog):
    p = provider(lambda request: httpx.Response(status, text="fake-secret"))
...
def test_invalid_payload_not_silently_swallowed(change):
    p = provider(lambda request: httpx.Response(200, json={"bars": {"SPY": [{**RAW, **change}]}}))
    with pytest.raises(ProviderError, match="invalid_historical_payload"):
        asyncio.run(p.get_historical_candles("SPY", "1h"))
    assert p.get_status().state == "degraded"


def test_backfill_paginates_all_pages_in_order():
    seen = []
    ...
    bars = asyncio.run(
        provider(handler).get_historical_candles(
...
def test_repeated_page_token_fails_instead_of_looping():
    p = provider(lambda request: httpx.Response(200, json={"bars": {}, "next_page_token": "loop"}))
    with pytest.raises(ProviderError, match="repeated_page_token"):
        asyncio.run(p.get_historical_candles("SPY", "1h"))


def test_duplicates_have_stable_identity_and_content_conflicts_keep_identity():
    p = provider()
...
def test_websocket_error_ack_is_not_success(stage, code, caplog):
    messages = [WELCOME, AUTH, SUB]
    messages[stage] = [{"T": "error", "code": code, "msg": "fake-secret"}]
    p = provider()
    with pytest.raises(ProviderError):
        asyncio.run(p.handshake(FakeSocket(messages)))
...
def test_missing_or_incomplete_ack_is_failure(messages):
    with pytest.raises(ProviderError, match="ack_missing"):
        asyncio.run(provider().handshake(FakeSocket(messages)))


def test_auth_and_subscription_acknowledged():
    p, socket = provider(), FakeSocket([WELCOME, AUTH, SUB])
...
def test_minute_ws_bar_never_emitted_as_hourly():
    async def scenario():
        socket = FakeSocket(
...
def test_reconnect_backoff_has_cap_and_safe_logs(caplog):
    delays = []
    ...
    async def scenario():
        p = provider(socket_factory=factory, sleep=sleep)
        with pytest.raises(asyncio.CancelledError):
            await anext(p.subscribe())
...
def test_backoff_resets_only_after_stable_connection():
    async def scenario():
        ...
        async def factory():
            ...
            if calls[0] < 3:
                raise OSError()
            return StableSocket([WELCOME, AUTH, SUB])
...
def test_weekend_holiday_early_close_and_evening_not_stalled(now):
    p = provider()
    p.clock = lambda: now
    p._state = "connected"
...
def test_calendar_dst_transition():
...
def test_settings_fail_early(values):
    with pytest.raises(ValidationError):
        Settings(_env_file=None, postgres_password=SecretStr("test"), **values)


def test_settings_normalize_symbols_and_hide_keys():
    s = Settings(
        _env_file=None,
...
def test_simulator_keeps_deterministic_price_chain():
...
def test_strategy_rejects_partial():
...
def test_smoke_skipped_without_network(monkeypatch, capsys):
...
def test_transport_failure_is_retryable_and_redacted(caplog):
    def handler(request):
        raise httpx.ConnectError("fake-secret", request=request)

    with pytest.raises(ProviderError, match="historical_transport") as error:
        asyncio.run(provider(handler).get_historical_candles("SPY", "1h"))
    assert error.value.retryable and "fake-secret" not in caplog.text


def test_unexpected_symbol_payload_fails():
    p = provider(lambda r: httpx.Response(200, json={"bars": {"TSLA": [RAW]}}))
    with pytest.raises(ProviderError, match="invalid_historical_payload"):
        asyncio.run(p.get_historical_candles("SPY", "1h"))


def test_price_beyond_database_precision_is_not_silently_rounded():
    p = provider(
...
def test_configuration_error_never_prints_secret_input():
...
def test_http_websocket_handshake_failure_is_classified(monkeypatch, status, retryable):
...
def test_opted_in_smoke_closed_session_is_bounded_and_skips_stream(monkeypatch, capsys):
    from scripts import smoke_test
